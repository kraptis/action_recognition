...
void test_yolo(char *cfgfile, char *weightfile, char *filename, float thresh)
{
    ...
    while(1){
        if(filename){
            ...
            printf("Press 1 for Image or 2 for Directory: \n");
            fflush(stdout);
            scanf("%d",&inp);
            fflush(stdout);
            printf("Press ctrl+D or ctrl+Z to continue");
	    while (getchar() != EOF);
            if(!inp)
	    {
               return;
        ...
        draw_detections(im, l.side*l.side*l.n, thresh, boxes, probs, voc_names, alphabet, 20);
        //save_image(im, "predictions"); // uncomment if you want to save the image with the bounding boxes
        //show_image(im, "predictions"); //uncomment if you want to display the image
